...
export const ProjectsLeftPanel: React.FC<ProjectsLeftPanelProps> = ({ isDarkMode, selectedProject, setSelectedProject }) => {
  ...
  return (
    <div className={`w-full lg:col-span-1 flex flex-col lg:pr-6 pb-6 lg:pb-0 lg:border-r ${isDarkMode ? 'lg:border-[#efeeee]' : 'lg:border-black'}`}>
        ...
        <nav aria-label="Project selection">
          <ul role="list">
            {PROJECTS.map((project) => {
              ...
              return (
                <li key={project} className={isActive ? 'mb-3 lg:mb-4' : 'mb-4 lg:mb-5'}>
                  <button
                    onClick={() => setSelectedProject(project)}
                    className={`w-full text-left transition-all duration-200 ease-in-out leading-none md:leading-tight
                      ${isActive
                        ? 'text-3xl sm:text-4xl md:text-6xl font-black tracking-tighter text-[#FF4500] no-cursor-invert'
                        : 'text-xl sm:text-2xl font-normal tracking-tight'
                      }`
                    }
